...
def cvt_int_to_na_dt(value):
   if not (value == '' or value is None):
        try:
           result =cvt_to_na_ts(value).date()
           return result
        except:
           return None
...
def cvt_int_to_es_ts(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(cvt_to_na_ts_3(value), is_dst=None).astimezone(tz).strftime(tm_fmt)
           return result
        except:
           return None

def cvt_int_to_na_ts(value):
   if not (value == '' or value is None):
        try:
           result = datetime.datetime.fromtimestamp(long(value)/1000000)
           return result
...
def cvt_int_to_es_dt(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(cvt_to_na_ts_5(value), is_dst=None).astimezone(tz).date()
           return result
        except:
           return None

def cvt_evt_to_es_dt(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(cvt_to_na_ts_6(value), is_dst=None).astimezone(tz).date()
           return result
        except:
           return None

def cvt_evt_to_es_ts(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(datetime.strptime(value,"%Y-%m-%d %H:%M:%S.%f"), is_dst=None).astimezone(tz).strftime(tm_fmt)
           return result
        except:
           return None

def cvt_evt_to_na_dt(value):
   if not (value == '' or value is None):
        try:
           result = cvt_to_na_ts_2(value).date()
           return result
        except:
           return None
...
tableName = "united.dfa2_activity_test"
...
udf_cvt_int_to_na_ts = udf(cvt_int_to_na_ts, TimestampType())
udf_cvt_int_to_na_dt = udf(cvt_int_to_na_dt, DateType())
udf_cvt_int_to_es_ts = udf(cvt_int_to_es_ts, StringType())
udf_cvt_int_to_es_dt = udf(cvt_int_to_es_dt, DateType())

udf_cvt_evt_to_es_ts = udf(cvt_evt_to_es_ts, StringType())
udf_cvt_evt_to_na_dt = udf(cvt_evt_to_na_dt, DateType())
udf_cvt_evt_to_es_dt = udf(cvt_evt_to_es_dt, DateType())

try:
	df = df1.withColumn("AcquiredTime", lit(currrent_time)).withColumn("md_interaction_time", 		udf_cvt_int_to_na_ts(df1._c19)).withColumn("md_interaction_time_loc", 	udf_cvt_int_to_es_ts(df1._c19)).withColumn("md_interaction_date", 		udf_cvt_int_to_na_dt(df1._c19)).withColumn("md_interaction_date_loc", 	udf_cvt_int_to_es_dt(df1._c19)).withColumn("md_event_time_loc", 		udf_cvt_evt_to_es_ts(df1._c106)).withColumn("md_event_date", 			udf_cvt_evt_to_na_dt(df1._c0)).withColumn("md_event_date_loc", 		udf_cvt_evt_to_es_dt(df1._c0))
...
try:
	datasource1 = datasource0.resolveChoice(
		specs=[('_c0', 'cast:long'),
			   ...
			   ('_c104', 'cast:long'),
			   ('_c105', 'cast:boolean'),
			   ('_c106', 'cast:timestamp'),
			   ('_c107', 'cast:timestamp'),
			   ('_c108', 'cast:int'),
...
applymapping1 = ApplyMapping.apply(frame=datasource1, mappings=[("_c0", "long", "event_time", "long"),
																...
																("_c58", "decimal(37,15)", "total_revenue", "decimal(37,15)"),
																...
																("_c79", "int", "dbm_cpm_fee_4_advertiser_currency", "int"),
																("_c80", "int", "dbm_cpm_fee_5_usd", "int"),
																("_c81", "int", "dbm_cpm_fee_5_partner_currency", "int"),
																("_c82", "int", "dbm_cpm_fee_5_advertiser_currency", "int"),
																("_c83", "int", "dbm_media_fee_1_usd", "int"),
																("_c84", "int", "dbm_media_fee_1_partner_currency","int"),
																("_c85", "int", "dbm_media_fee_1_advertiser_currency", "int"),
																("_c86", "int", "dbm_media_fee_2_usd", "int"),
