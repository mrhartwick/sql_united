...
try:
	df = df1.withColumn("AcquiredTime", lit(currrent_time)).withColumn("md_event_time_loc", udf_cvt_to_es_ts(df1._c103).cast("timestamp")).withColumn("md_event_date", udf_cvt_to_na_dt(df1._c103)).withColumn("md_event_date_loc", udf_cvt_to_es_dt(df1._c103).cast("date"))
except:
...
try:
	datasource1 = datasource0.resolveChoice(
		specs=[('_c0', 'cast:long'),
			   ...
			   ('_c101', 'cast:long'),
			   ('_c102', 'cast:boolean'),
			   ('_c103', 'cast:timestamp'),
			   ('_c104', 'cast:timestamp'),
			   ('_c105', 'cast:int'),
