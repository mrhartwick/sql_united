create table diap01.mec_us_united_20056.ual_freq_dbm_cost_wk
(
...
insert into diap01.mec_us_united_20056.ual_freq_dbm_cost_wk
(user_id,dbm_cost,imps,clicks,con,vew_con,clk_con,tix,vew_tix,clk_tix,rev,vew_rev,clk_rev)

(
    select
        t3.user_id,
                      avg(t3.dbm_cost)                              as dbm_cost,
              avg(t3.imps)                                  as imps,
              avg(t3.clicks)                                as clicks,
              isNull(avg(t3.con),cast(0 as int))            as con,
              isNull(avg(t3.vew_con),cast(0 as int))        as vew_con,
              isNull(avg(t3.clk_con),cast(0 as int))        as clk_con,
              isNull(avg(t3.tix),cast(0 as int))            as tix,
              isNull(avg(t3.vew_tix),cast(0 as int))        as vew_tix,
              isNull(avg(t3.clk_tix),cast(0 as int))        as clk_tix,
              isNull(avg(t3.rev),cast(0 as decimal(20,10))) as rev,
              isNull(avg(t3.vew_rev),cast(0 as int))        as vew_rev,
              isNull(avg(t3.clk_rev),cast(0 as int))        as clk_rev
