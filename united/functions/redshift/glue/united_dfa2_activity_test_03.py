...
zip_ref = zipfile.ZipFile('pythonmodules_new.zip', 'r')
...
# Chaining these functions doesn't seem to work within the withColumn method, so each
# one contains entire calculation.

tm_fmt = "%Y-%m-%d %H:%M:%S:%f"
dt_fmt = "%Y-%m-%d"

#This function is used to convert Unix epoch to timestamp
...
#This function is used to convert Unix epoch to date
def convert_to_na_dt(value):
   if not (value == '' or value is None):
        try:
           result = datetime.datetime.fromtimestamp(long(value)/1000000).date()
           return result
        except:
           return None

# This function is used to convert Unix epoch to EST (GMT -5) timestamp
def convert_to_es_ts(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(datetime.datetime.fromtimestamp(long(value)/1000000), is_dst=None).astimezone(tz)
           return result
        except:
           return None

#This function is used to convert Unix epoch to EST (GMT -5) date
def convert_to_es_dt(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(datetime.datetime.fromtimestamp(long(value)/1000000), is_dst=None).astimezone(tz).date()
           return result
        except:
           return None
...
tableName = "united.dfa2_activity_test_03"
...
udf_cvt_to_es_dt = udf(convert_to_es_dt, StringType())
