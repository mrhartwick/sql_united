...
def convert_int_to_na_dt(value):
   if not (value == '' or value is None):
        try:
           result = convert_to_na_ts_2(value).date()
           return result
        except:
           return None
...
def convert_int_to_es_ts(value):
   if not (value == '' or value is None):
        try:
           ...
           result = pytz.utc.localize(convert_to_na_ts_3(value), is_dst=None).astimezone(tz).strftime(tm_fmt)
           return result
        except:
           return None
...
def convert_int_to_es_dt(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(convert_to_na_ts_4(value), is_dst=None).astimezone(tz).date()
           return result
        except:
           return None
...
def convert_evt_to_na_dt(value):
   if not (value == '' or value is None):
        try:
           result = convert_to_na_ts_6(value).date()
           return result
        except:
           return None
...
def convert_evt_to_es_ts(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(convert_to_na_ts_6(value), is_dst=None).astimezone(tz).strftime(tm_fmt)
           return result
        except:
           return None
...
def convert_evt_to_es_dt(value):
   if not (value == '' or value is None):
        try:
    	   tz = timezone('America/New_York')
           result = pytz.utc.localize(convert_to_na_ts_7(value), is_dst=None).astimezone(tz).date()
           return result
        except:
           return None
...
try:
    print 'Schema'
    df1.printSchema()
    df = df1.withColumn("AcquiredTime", lit(currrent_time)).withColumn("md_interaction_time", udf_cvt_int_to_na_ts(df1._c19)).withColumn("md_interaction_time_loc", udf_cvt_int_to_es_ts(df1._c19).cast("timestamp")).withColumn("md_interaction_date", udf_cvt_int_to_na_dt(df1._c19)).withColumn("md_interaction_date_loc", udf_cvt_int_to_es_dt(df1._c19).cast("date")).withColumn("md_event_time_loc", 		udf_cvt_evt_to_es_ts(df1._c0).cast("timestamp")).withColumn("md_event_date", udf_cvt_evt_to_na_dt(df1._c0)).withColumn("md_event_date_loc", udf_cvt_evt_to_es_dt(df1._c0).cast("date"))
