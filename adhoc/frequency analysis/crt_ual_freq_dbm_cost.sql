create procedure dbo.crt_ual_freq_dbm_cost
as
if OBJECT_ID('master.dbo.ual_freq_dbm_cost',N'U') is not null
    drop table master.dbo.ual_freq_dbm_cost;


create table master.dbo.ual_freq_dbm_cost
(
...
insert into master.dbo.ual_freq_dbm_cost
...
from (select
    dcmreport.user_id as user_id,
          ...
          lag(dcmReport.plce_id,1,0) over (
              order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lagplcnbr,
          ...
          lag(sum(dcmreport.dbm_cost),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_dbm_cost,
          sum(dcmreport.impressions)                                                           as impressions,
          sum(dcmreport.clicks)                                                                as clicks,
          sum(dcmreport.vew_conv)                                                                  as vew_con,
          lag(sum(dcmreport.vew_conv),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_vew_con,
          sum(dcmreport.clk_conv)                                                                  as clk_con,
          lag(sum(dcmreport.clk_conv),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_clk_con,
          sum(dcmreport.vew_tix)                                                                   as vew_tix,
          lag(sum(dcmreport.vew_tix),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_vew_tix,
          sum(dcmreport.clk_tix)                                                                   as clk_tix,
          lag(sum(dcmreport.clk_tix),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_clk_tix,
          sum(dcmreport.vew_rev)                                                                   as vew_rev,
          lag(sum(dcmreport.vew_rev),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_vew_rev,
          sum(dcmreport.clk_rev)                                                                   as clk_rev,
          lag(sum(dcmreport.clk_rev),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_clk_rev,
          sum(dcmreport.rev)                                                                   as rev,
          lag(sum(dcmreport.rev),1,0) over ( order by dcmreport.dcmdate asc,dcmreport.plce_id asc,dcmreport.site_id_dcm, dcmreport.user_id desc) as lag_rev
