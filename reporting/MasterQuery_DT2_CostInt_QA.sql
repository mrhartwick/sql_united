...
--      dcm impressions (for comparison (qa) to dcm console)
        ...
        sum(cst.iv_imps)                                                              as iv_impressions_2,
        ...
        sum(cst.dv_imps)                                                              as dv_impressions_2,
        ...
        sum(cst.mt_imps)                                                              as mt_impressions_2,
...
--      fields to be used followed by fields from cst; fields should match _chk fields
        sum(case when t2.costmethod like 'dCPM' then cst.clk_con else t2.clk_con end) as clk_con,
        sum(cst.clk_con)                                                              as clk_con_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.clk_rev else t2.clk_rev end) as clk_rev,
        sum(cst.clk_rev)                                                              as clk_rev_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.clk_tix else t2.clk_tix end) as clk_tix,
        sum(cst.clk_tix)                                                              as clk_tix_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.con else t2.con end)         as tot_con,
        sum(cst.con)                                                                  as tot_con_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.rev else t2.rev end)         as tot_rev,
        sum(cst.rev)                                                                  as tot_rev_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.tix else t2.tix end)         as tot_tix,
        sum(cst.tix)                                                                  as tot_tix_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.vew_con else t2.vew_con end) as vew_con,
        sum(cst.vew_con)                                                              as vew_con_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.vew_rev else t2.vew_rev end) as vew_rev,
        sum(cst.vew_rev)                                                              as vew_rev_chk,
        sum(case when t2.costmethod like 'dCPM' then cst.vew_tix else t2.vew_tix end) as vew_tix,
        sum(cst.vew_tix)                                                              as vew_tix_chk
...
--        amobee video 360 placements, tracked differently across dcm, innovid, and moat; this combines the three placements into one
           ...
           sum(cast(t1.revenue as decimal(10,2))) as rev,
