...
--     case
--     when cast(t3.dcmdate as date) between '2017-10-17' and '2017-10-08' then 'week 1'
...
-- @report_ed date;
-- --
-- set @report_ed = '2017-10-17';
-- set @report_st = '2017-12-12';
           select
...
-- case
-- when t1.placement_id in (206798246,206798393,206798399,206831200,206831218,206831614,206831647,206831659,206831668,206832295,206856135,206856138,206856174,206856177,207902802,207902805,207828554,207827324,207827939,207881356,207828551) then 'DEN'
-- when t1.placement_id in (206831191,206831209,206831212,206831653,206831656,206832280,206832286,206832289,206832292,206855814,206855817,206856147,206856153,206856159,206856165,207881353,207828545,207902301,207881350,207902787,207827321,207881728) then 'EWR'
-- when t1.placement_id in (206798249,206798369,206798375,206831605,206831638,206832265,206832274,206855790,206855796,206855799,206855802,206855808,206856141,206856150,207902808,207881359,207828557,207828542,207828548,207881734,207881725) then 'SFO' else 'Other' end as City,

-- case
-- when t1.placement_id in (206798393,206831200,206831647,206831659,206856135,206856138,206856177,206831191,206831212,206832289,206832292,206855814,206855817,206856159,206856165,206798369,206798375,206832265,206855796,206855808,206856141,206856150) then 'Registered'
-- when t1.placement_id in (206798246,206798399,206831218,206831614,206831668,206832295,206856174,206831209,206831653,206831656,206832280,206832286,206856147,206856153,206798249,206831605,206831638,206832274,206855790,206855799,206855802) then 'Unregistered' else 'NA' end as Reg_Unreg,
...
,sum(case when activity_id = 978826 and (ta.conversion_id = 1 or ta.conversion_id = 2) and ta.total_revenue <> 0 then 1 else 0 end ) as con
...
where cast (timestamp_trunc(to_timestamp(interaction_time / 1000000),''SS'') as date ) between ''2017-10-17'' and ''2017-12-12''
and not regexp_like(substring(other_data,(instr(other_data,''u3='') + 3),5),''mil.*'',''ib'')
and (activity_id = 978826 or activity_id = 1086066)
and (campaign_id = 10742878 and placement_id in (207163233,207090170,207163230,207160432,207162882,207091082,207162885,207160429))
and (advertiser_id <> 0)
...
where cast (timestamp_trunc(to_timestamp(event_time / 1000000),''SS'') as date ) between ''2017-10-17'' and ''2017-12-12''
and (campaign_id = 10742878 and placement_id in (207163233,207090170,207163230,207160432,207162882,207091082,207162885,207160429))
...
--         and ((t1.campaign_id = 20377442) or (t1.campaign_id = 10742878 and t1.site_id_dcm = 1190273 and t1.placement like '%_WinHubs_%'))
