...
-- ,final.site                                                   as site
,sum(final.unq_led)                                           as unq_leads
...
from (
        ...
        select
           ...
           ,sum(report.unq_vew_led)                               as unq_vew_led
           ,sum(report.unq_clk_led)                               as unq_clk_led
           ,sum(report.unq_vew_led) + sum(report.unq_clk_led)     as unq_led
           ,report.other_data
...
--                    , cast(subString(conversions.other_data,( INSTR(conversions.other_data,'u9=') + 3 ),10) as date) as traveldate_1
                      ...
                      ,sum(case when conversions.activity_id = 1086066 and conversions.conversion_id = 1 then 1 else 0 end) as unq_clk_led
                      ,sum(case when conversions.activity_id = 1086066 and conversions.conversion_id = 2 then 1 else 0 end) as unq_vew_led


                from (

                        select distinct(user_id), activity_id, conversion_id, interaction_time, campaign_id, site_id_dcm, placement_id, other_data
