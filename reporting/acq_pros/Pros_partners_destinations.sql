...
from
    (
        select
            ...
            when t1.rt_2_dest in ('SYD','MEL','PEK','PVG','CTU','XIY','HKG','EZE','SCL','LHR','DUB','GVA','EDI','FRA','CDG','HNL','KOA','ITO','LIH','AMS','SIN','AKL','MUC','ZRH','ICN','HAM','KIX','BRU','OGG','NGO') then t1.rt_2_dest
            when t1.rt_1_dest in ('SYD','MEL','PEK','PVG','CTU','XIY','HKG','EZE','SCL','LHR','DUB','GVA','EDI','FRA','CDG','HNL','KOA','ITO','LIH','AMS','SIN','AKL','MUC','ZRH','ICN','HAM','KIX','BRU','OGG','NGO') then t1.rt_1_dest
            else t1.rt_2_dest
            end                                                                                          as destination,
...
from
    (
        select
            ...
            case when (len(ISNULL(t1.rt_2_dest,'')) = 0) then t1.rt_1_dest
            when t1.rt_1_orig = t1.rt_2_dest and t1.rt_2_orig = t1.rt_1_dest then t1.rt_1_dest
            else t1.rt_2_dest end                                                                        as destination,
            t1.rt_1_dest                                                                                 as route_1_destination,
            t1.rt_2_dest                                                                                 as route_2_destination,
            t1.rt_1_orig                                                                                 as route_1_origin,
            t1.rt_2_orig                                                                                 as route_2_origin,
            ...
            cast(prs.rate as decimal(10,2))                                                              as rate,
            sum(t1.impressions)                                                                          as impressions,
            case when t1.site_id_dcm = 3267410 -- Quantcast
                then cast((sum(cast(impressions as decimal(20,10))) * cast(prs.rate as decimal(20,10))) as
                          decimal(20,10)) * 0.732853744    end                                           as cost,
            sum(t1.vew_led)                                                                              as vew_led,
            sum(t1.clk_led)                                                                              as clk_led,
            sum(t1.led)                                                                                  as led,
            sum(t1.vew_con)                                                                              as vew_con,
