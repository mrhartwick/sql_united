...
select
  ...
  t3.placement                                                                                    as placement,
      case when t3.placement like '%Video%' or t3.placement like '%[_]VID[_]%' then 'Video' else 'Standard'  end      as "Asset Type",
  ...
  t3.planned_amt                                                                                       as "planned amt",
    t3.planned_amt/max(t3.newcount) as "planned amt2",
--  t3.planned_cost                                                                                       as "planned cost",
  ...
  sum(t3.iv_impressions)                                                                               as "innovid impressions",
    sum(t3.IV_Completes) 																				as "Innovid Video Completes",
...
sum(t3.billrevenue)                                                                                      as billable_revenue,
  sum(t3.adjsrevenue)                                                                                  as "adjusted (t3) revenue"
...
--         not subject to viewability
             when (t2.dv_map = 'N') and (t2.costmethod = 'dCPM')
               then cast((c1.vew_rev) + c1.clk_rev as decimal(10,2))
             when (t2.dv_map = 'N')
...
where cast (timestamp_trunc(to_timestamp(interaction_time / 1000000),''SS'') as date ) between ''2017-01-23'' and ''2017-02-13''
...
where cast (timestamp_trunc(to_timestamp(event_time / 1000000),''SS'') as date ) between ''2017-01-23'' and ''2017-02-13''
and campaign_id in (10768497) -- Polaris 2017
