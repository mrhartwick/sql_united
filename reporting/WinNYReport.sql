...
--         not subject to viewability
             when (t2.dv_map = 'N')
               then cast((t2.vew_rev * .08) + t2.clk_rev as decimal(10,2))
...
--         subject to viewability with flag; mt source
             ...
             (((t2.vew_rev) *
                          (cast(mt.groupm_passed_impressions as decimal) /
                            nullif(cast(mt.total_impressions as decimal),0))) * .08) + t2.clk_rev as decimal(10,2))
...
--         subject to viewability; dv source
             ...
             (((t2.vew_rev) *
                (cast(dv.groupm_passed_impressions as decimal) /
                            nullif(cast(dv.total_impressions as decimal),0))) * .08) + t2.clk_rev as decimal(10,2))
