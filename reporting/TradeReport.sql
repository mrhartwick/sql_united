...
---differentiation between the three trade campaigns: Trade, SME, MICE
...
--asset type
  case when t3.placement  like '%Video%' or t3.placement like '%[_]VID[_]%' then 'Video'
  when t3.site_id_dcm= 1290241 and t3.placement like '%eNewsletter%' then 'Standard'
    when t3.site_id_dcm= 3512186 and t3.placement like '%Newsletter%' then 'Standard'
  when t3.placement  like '%Email%' or t3.placement like '%Newsletter%' or t3.placement like '%E-blast%' then 'Email'
        else 'Standard'  end                                                                           as "Asset Type",
...
-- reference/optional: planned package end date, from prisma; attributed to all placements within package.
    ...
    sum(t3.iv_completes) as iv_completes,
...
--         not subject to viewability, DBM
             ...
             when (t2.dv_map = 'N' and t2.costmethod = 'dCPM' and placement LIKE '%First and Business%%')
              ...
              when (t2.dv_map = 'N' and t2.costmethod = 'dCPM')
              then cast(((cst.vew_rev + cst.clk_rev)  * .08) *.9 as decimal(10,2))

--         not subject to viewability
             when (t2.dv_map = 'N')
               then cast(((t2.vew_rev + t2.clk_rev) * .08) *.9 as decimal(10,2))
...
where cast (timestamp_trunc(to_timestamp(interaction_time / 1000000),''SS'') as date ) between ''2017-03-10'' and ''2017-09-25''
...
where cast (timestamp_trunc(to_timestamp(event_time / 1000000),''SS'') as date ) between ''2017-03-10'' and ''2017-09-25''
and campaign_id in (11069476) -- display 2017
