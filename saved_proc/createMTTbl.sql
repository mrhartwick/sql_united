...
insert into master.dbo.MTTable
    select distinct
        replace(left(t1.placement_name,6)+'_'+
                case
                when (t1.media_property like '%[Cc]hicago%[Tt]ribune%' or t1.media_property like '[Tt]ribune_[Ii]nteractive%') then 'ChicagoTribune'
                ...
                when t1.media_property like '%[Ww]all_[Ss]treet_[Jj]ournal%' then 'Wall Street Journal'
